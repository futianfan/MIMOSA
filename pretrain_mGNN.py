...
def train(args, model_list, loader, optimizer_list, device):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        print("accuracy: {:3.2f}%".format(acc_node*100))
...
class Linear(torch.nn.Module):
    def __init__(self, input_dim, output_dim, device):
        ...
        self.W = torch.nn.Linear(input_dim, output_dim).to(device)
...
def main():
    ...
    for epoch in range(1, args.epochs+1):
        ...
        torch.save(model.state_dict(), args.output_model_file + "_epoch" + str(epoch) + ".pth")
        torch.save(linear_pred_atoms.state_dict(), args.output_model_file + "_epoch_" + str(epoch) + "_linear_atom.pth")
        torch.save(linear_pred_bonds.state_dict(), args.output_model_file + "_epoch_" + str(epoch) + "_linear_bond.pth")
        print("save ok")
