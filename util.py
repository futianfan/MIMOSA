...
from rdkit.Chem import AllChem, MolToSmiles, MolFromSmiles, Kekulize, Descriptors
...
def similarity(smiles_a, smiles_b):
    ...
    ''' 
    mol_a = MolFromSmiles(smiles_a)
    mol_b = MolFromSmiles(smiles_b)

    if mol_a is None or mol_b is None:
        return 0.0 

    fps_a = Chem.RDKFingerprint(mol_a)
    fps_b = Chem.RDKFingerprint(mol_b)
    return DataStructs.FingerprintSimilarity(fps_a, fps_b)
...
class MaskAtom:
    ...
    def __call__(self, data, masked_atom_indices=None):
        ...
        * mask rate number of atom indices
        ...
        debugging only)
        :return: None, Creates new attributes in original data object:
        data.mask_node_idx == data.masked_atom_indices
        data.mask_node_label
        data.mask_edge_idx == connected_edge_indices
