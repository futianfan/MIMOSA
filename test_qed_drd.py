...
from props import drd2, qed, penalized_logp
...
def evaluate(input_smiles, candidate_smiles, input_qed, input_drd, \
                coefficient_similarity = 1.0, coefficient_drd = 0.3, coefficient_qed = 0.3, coefficient_logp = 0.3):
...
def judge(input_smiles, candidate_smiles, input_qed, input_drd, threshold_similarity = 0.3, threshold_qed = 0.82, qed_gap_threshold = 0.1, logp_gap_threshold = 0.3, \
             threshold_drd = 0.7, drd_gap_threshold = 0.1):
    sim = similarity(input_smiles, candidate_smiles)
    input_qed = qed(input_smiles)
    qed_score = qed(candidate_smiles)
    input_drd = drd2(candidate_smiles)
    ...
    return sim >= threshold_similarity \
            and (qed_score >= threshold_qed or qed_score - input_qed >=qed_gap_threshold) \
...
def multiple_iteration(input_smiles, model_list, topk = 5, max_smiles = 20, iteration = 2, is_sort=True, is_print=True):
    ...
    for i in range(iteration):
        ...
        ### sort by posterior 
        if is_sort:
            candidate_smiles_list = [(smiles, evaluate(input_smiles, smiles, input_qed, input_drd)) for smiles in candidate_smiles_list]
        ...
        for candidate_smiles in candidate_smiles_list:
            if judge(input_smiles, candidate_smiles, input_qed, input_drd):
...
def test_single_smiles(input_smiles, model_list):
    qed_score = qed(input_smiles)
    drd_score = drd2(input_smiles)
    success_list = multiple_iteration(input_smiles, model_list)
    for smiles in success_list:
        print(smiles, "similar: {:1.3f}; origin qed: {:1.2f}; origin drd: {:1.2f};\
                qed improvement: {:1.2f}, drd improvement: {:2.2f}".format(\
                similarity(input_smiles, smiles), \
                qed_score,\
                drd_score, \
                qed(smiles) - qed_score, \
...
def main():
    ...
    #input_smiles = "CC(=O)Nc1ccc(OC[C@H](O)CN2CCc3sccc3C2)cc1"
    input_smiles = "CC(C)(C)[C@H]1CCc2c(sc(NC(=O)COc3ccc(Cl)cc3)c2C(N)=O)C1"
    input_smiles = "CCNC(=O)c1ccc(NC(=O)N2CC[C@H](C)[C@H](O)C2)c(C)c1"
    input_smiles = "CCc1nc(-c2ccc(C(=O)NCc3cccnc3)cc2)cs1"
    input_smiles = "O=C(Cn1cncn1)N[C@H]1CCc2[nH+]ccn2C1"
    #print("original", qed(input_smiles))
    #proposal_smiles = single_iteration(input_smiles, model_list, topk = topk)
    #success_list = multiple_iteration(input_smiles, model_list)
    #for smiles in success_list:
    #    print(smiles, "similar: {:1.3f}; qed: {:1.3f}, logp improvement: {:2.2f}".format(\
    #            similarity(input_smiles, smiles), qed(smiles), penalized_logp(smiles) - logp_score))
    with open("qed_plogp2.txt", 'r') as fin: 
        lines = fin.readlines() 
    for line in lines:
        input_smiles = line.strip() 
        ll = test_single_smiles(input_smiles, model_list)
        if (ll >= 3):
            results_append("qed_drd_success.txt", input_smiles)
