...
def multiple_iteration(input_smiles, model_list, topk = 5, max_smiles = 20, iteration = 2, is_sort=True, is_print=True):
    ...
    for i in range(iteration):
        ...
        ### sort by posterior 
        if is_sort:
            candidate_smiles_list = [(smiles, evaluate(input_smiles, smiles)) for smiles in candidate_smiles_list]
        ...
        for candidate_smiles in candidate_smiles_list:
            if judge(input_smiles, candidate_smiles):
...
def test_single_smiles(input_smiles, model_list):
    ...
    for smiles in success_list:
        print(smiles, "similar: {:1.3f}; origin qed: {:1.3f}; qed improvement: {:1.2f}, logp improvement: {:2.2f}".format(\
                similarity(input_smiles, smiles), qed_score,\
...
def main():
    ...
    for line in lines:
        ...
        if (ll >= 3):
            results_append("qed_plogp_success.txt", input_smiles)

if __name__ == "__main__":

    #main()
    input_smiles = "O=C(Cn1cncn1)N[C@H]1CCc2[nH+]ccn2C1"
    print(drd2(input_smiles)) 
